...
### در ابتدا تعداد متغیرها به ورودی داده می شود. سپس تعداد مینترم ها را از ورودی دریافت می کند و پس از آن، مینترم ها را از ورودی میگیرد.
...
### 3. کتابخانه set:
+ یک مجموعه از داده‌ها که هیچ عنصر تکراری ندارد و به‌صورت مرتب‌شده ذخیره می‌شود
 + set<Type> name;



### 4.  کتابخانه algorithm:
+ مجموعه‌ای از توابع سربارگذاری‌ شده‌ی عمومی (generic) است که روی انواع داده مثل vector, array, set, map و ... قابل استفاده‌اند.
+ در مرتب سازی، جستجو، حداقل/حداکثر، معکوس کردن، حذف عناصر تکراری و ... به کار می رود.



### 5. کتابخانه climits:
 + شامل ثابت‌های عددی هست که مربوط به محدودیت‌های انواع داده‌ی صحیح هستند.
